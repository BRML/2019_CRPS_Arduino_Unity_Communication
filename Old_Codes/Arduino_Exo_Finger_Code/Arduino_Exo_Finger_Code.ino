...
// Define Struct for the Assistive Force
struct sAssistiveForce
{
  double Force_Level = 0;
  double Direction = 0.5; 
  double Threshold_Position = 0.15; 
};
...
struct sJoint{
  ...
  float MAX_Force;     //[N] Maximum allowable force for this joint
  float radius = 1;
  ...
  struct sAssistiveForce Assistance_Force;
...
float MAX_Assistance_Force = 0; // assistance force scalable with slider
int force_raw = 0;
...
void loop()
{
  ...
  if(controllerAttached){
    ...
    MAX_Assistance_Force=-(analogRead(potentiometerPin)/1023.0-1)*10;  // Read Potentiometer Value 
  ...
  Generate_Assistive_Force();  // Calculate Assistance Force
...
void runAdmittance(){
  for(int i = 0; i < num_Joints; i++){
    if (joints[i].isOn){   // Can be used to turn certain joints off
      if(abs(joints[i].force+joints[i].Assistance_Force.Force_Level)<joints[i].MAX_Force){  // Check if we are using more than max force, if we do stop

        joints[i].ddx = 1/joints[i].mass * ((joints[i].force+joints[i].Assistance_Force.Force_Level)*joints[i].radius - joints[i].damping * joints[i].dx);
...
void sumMeasurements(){
  ...
  Average_Assistance_Force += MAX_Assistance_Force;
...
void outputData(){
  ...
  addOutString(Average_Assistance_Force);
...
// FUNCTION : Generates an assistive force
void Generate_Assistive_Force(){
         
  for(int i = 0; i < num_Joints; i++){
     if(joints[i].x > joints[i].Assistance_Force.Threshold_Position && joints[i].Assistance_Force.Direction == 0.5 )
        joints[i].Assistance_Force.Direction = 1;
    
     if(joints[i].x < (1-joints[i].Assistance_Force.Threshold_Position) && joints[i].Assistance_Force.Direction == -0.5 ) 
        joints[i].Assistance_Force.Direction = -1;
    
     if(joints[i].x  == joints[i].MAX_Pos && joints[i].Assistance_Force.Direction == 1) 
        joints[i].Assistance_Force.Direction = -0.5; 
      
     if(joints[i].x  == joints[i].MIN_Pos && joints[i].Assistance_Force.Direction == -1)
        joints[i].Assistance_Force.Direction = 0.5; 
    
     if(joints[i].Assistance_Force.Force_Level < MAX_Assistance_Force && joints[i].Assistance_Force.Direction ==  1)
        joints[i].Assistance_Force.Force_Level = joints[i].Assistance_Force.Force_Level + 0.001;
    
     if(joints[i].Assistance_Force.Force_Level > -MAX_Assistance_Force && joints[i].Assistance_Force.Direction == -1)
        joints[i].Assistance_Force.Force_Level = joints[i].Assistance_Force.Force_Level - 0.001;
    
     if(joints[i].Assistance_Force.Direction == 0.5 || joints[i].Assistance_Force.Direction == -0.5)
        joints[i].Assistance_Force.Force_Level = 0;
  }
